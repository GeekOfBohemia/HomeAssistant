...
from appframe.type_def import DictListDef, DictType, StrType, StateType
from appframe.helpers import OFF, AutoName, HelperOp
from appframe.ws_comm import CmdType
...
ws_oper_def: DictListDef = {
    ...
    "input_button": {"type": "input_button/create", "name": "", "icon": None},
    "input_button_delete": {
        "type": "input_boolean/delete",
        "input_boolean_id": "",
    },
...
AREA_ID: Final = "area_id"


@dataclass
class EntityObject:
    id: str = field(default_factory=HelperOp.get_id)
    name: StrType = None
    area: StrType = None
    domain: StrType = None
    ...
    cmd_update_register: CmdType = None
    ...
    def __post_init__(self):
        ...
        if self.domain is not None:
            ...
            self.entity_id = HelperOp.entity_id(self.domain, self.name)
        elif self.name is not None:
            self.entity_id = self.name
            self.domain, self.name = HelperOp.split_entity(self.name)
        ...
        if self.initial is None:
            ...
            if self.domain == "input_number":
                self.initial = self.attributes.get("min", 0)
        if self.initial is None:
            self.initial = ""
        ...
        if self.area is None:
            self.area = h.remove_key(self.attributes, AREA_ID, "")
        ...
        if self.area is not None and len(self.area) > 0:
            self.attributes.update({AREA_ID: self.area})
        ...
        if self.domain == "input_boolean":
            if h.yes(value):
                self.hass.sync_call_service(
                    f"{self.domain}/turn_on", entity_id=self.entity_id
                )
            else:
                self.hass.sync_call_service(
                    f"{self.domain}/turn_off", entity_id=self.entity_id
                )
        elif self.domain == "input_button":
            if h.yes(value):
                self.hass.sync_call_service(
                    f"{self.domain}/press", entity_id=self.entity_id
                )
        else:
            self.debug(f"{self.domain}/set_value - {value}")
            self.hass.sync_call_service(
...
class EntityOper(HassBasicApp):
    ...
    def _get_cmd_update(self, entity_object: EntityObject) -> Cmd:
        ...
        _, id_name_var = HelperOp.split_entity(entity_object.entity_id)
        ...
        if entity_object.attributes is not None:
            ...
            area = entity_object.attributes.get(AREA_ID)
        if icon is not None:
            cmd_data.update({"icon": icon})
        if area is not None:
            cmd_data.update({AREA_ID: area})
        self.debug(cmd_data)
        cmd = Cmd(
    ...
    def _get_cmd_create(self, entity_object: EntityObject) -> CmdType:
        if entity_object.domain in ws_oper_def.keys():
            ...
            HelperOp.update_strict(cmd_data, entity_object.attributes)
            ...
            for k, val in cmd_data.items():
                if val is None:
                    to_delete.append(k)
            for d in to_delete:
                HelperOp.remove_key(cmd_data, d)
